...
import type { IWorkshopFeedbackRepository } from "../repositories/workshop-feedback.repository.interface";
...
export class WorkshopFeedbackService implements IWorkshopFeedbackService {
  ...
  async submitFeedback(
    ...
    try {
      ...
      return success({ feedbackId: feedback.id });
    } catch (error) {
      return handleError(
        error,
    ...
    try {
      ...
      return success({
        feedbacks: feedbacks.map((f) => ({
          ...
          createdAt: f.createdAt,
          publicName: f.isAnonymous
            ? "Étudiant anonyme"
            : f.apprentice?.user?.name || "Anonyme",
          realName: f.apprentice?.user?.name || null,
          realEmail: f.apprentice?.user?.email || null,
          apprenticeId: f.apprenticeId,
          mentorId: f.mentorId,
          mentorName: f.mentor?.user?.name || null,
          workshopId: f.workshopId,
          workshopTitle: f.workshop?.title || null,
        })),
        total,
      });
    } catch (error) {
