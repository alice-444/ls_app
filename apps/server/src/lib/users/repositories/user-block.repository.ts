...
export class PrismaUserBlockRepository implements IUserBlockRepository {
  async create(blockerId: string, blockedId: string): Promise<UserBlockEntity> {
    const block = await (prisma as any).user_block.create({
      data: {
  ...
  async delete(blockerId: string, blockedId: string): Promise<void> {
    await (prisma as any).user_block.deleteMany({
      where: {
        blockerId,
        blockedId,
      },
  ...
  async findBlock(
    ...
    const block = await (prisma as any).user_block.findUnique({
      where: {
        blockerId_blockedId: {
          blockerId,
          blockedId,
  ...
  async findBlocksByBlocker(blockerId: string): Promise<UserBlockEntity[]> {
    const blocks = await (prisma as any).user_block.findMany({
      where: { blockerId },
