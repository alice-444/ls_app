...
import { POST, DELETE } from "@/app/api/profile/publish/route";
...
describe("POST /api/profile/publish", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    const req = createRequest("/api/profile/publish", { method: "POST" });
...
describe("DELETE /api/profile/publish", () => {
  beforeEach(() => {
    vi.mocked(getAuthenticatedSession).mockResolvedValue({
      ok: false,
      response: NextResponse.json({ error: "Unauthorized" }, { status: 401 }),
  ...
  it("returns 401 when not authenticated", async () => {
    const req = createRequest("/api/profile/publish", { method: "DELETE" });
