...
import type {
  IWorkshopQueryService,
  IWorkshopApprenticeQueryService,
} from "./workshop-query.service.interface";
...
import { WorkshopResponseDTO } from "../../dto/workshop.dto";
import {
  mapWorkshopToPublicDTO,
  mapWorkshopsToPublicDTOs,
} from "../../utils/workshop-public-dto.mapper";
...
export class WorkshopQueryService implements IWorkshopQueryService {
  ...
  async getWorkshopsByCreator(
    ...
    try {
      ...
      return success(
        mapWorkshopsToPublicDTOs(workshops, this.workshopVideoLinkService),
      );
    } catch (error) {
      return handleError(
        error,
  ...
  async getWorkshopById(
    ...
    try {
      ...
      return success(
        mapWorkshopToPublicDTO(workshop, this.workshopVideoLinkService),
      );
    } catch (error) {
      return handleError(
        error,
...
export class WorkshopApprenticeQueryService implements IWorkshopApprenticeQueryService {
  constructor(
    private readonly workshopRepository: IWorkshopRepository,
    private readonly accessGuard: IWorkshopAccessGuard,
