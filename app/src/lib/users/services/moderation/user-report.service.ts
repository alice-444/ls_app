...
import type { IUserReportRepository } from "../../repositories/moderation/user-report.repository.interface";
...
export class UserReportService implements IUserReportService {
  ...
  async createReport(
    ...
    const { reporterUserId, reportedUserId, reason, details, messageId } =
      input;
    ...
    try {
      const reporter =
        await this.appUserRepository.findByUserId(reporterUserId);
      if (!reporter)
        return failure("Utilisateur (rapporteur) introuvable", 404);

      const reported =
        await this.appUserRepository.findByUserId(reportedUserId);
      ...
      if (this.notificationService) {
        const reporterName =
          await this.appUserRepository.findUserNameByUserId(reporterUserId);
        const reportedName =
          await this.appUserRepository.findUserNameByUserId(reportedUserId);
  ...
  async getReportsByReporter(userId: string): Promise<Result<any[]>> {
    try {
      ...
      return handleError(
        error,
        createErrorContext("getReportsByReporter", { userId }),
      );
    }
  }

  async getAdminReportQueue(params?: {
    limit?: number;
    offset?: number;
    status?: string;
  }): Promise<Result<any[]>> {
  ...
  async reviewReport(
    ...
    try {
      ...
      return handleError(
        error,
        createErrorContext("reviewReport", { resourceId: reportId }),
      );
    }
  }
  ...
  async bulkReviewReports(
    ...
    try {
      ...
      const result = await this.userReportRepository.updateStatuses(
        reportIds,
        status,
        admin.id,
        adminNotes || null,
      ...
      logger.info("Reports bulk reviewed", {
        count: result.count,
        status,
        reviewedBy: adminUserId,
      });
      ...
      if (this.auditLogService) {
        await this.auditLogService.record({
          adminId: adminUserId,
          action: "USER_REPORTS_BULK_REVIEWED",
          details: {
            reportIds,
            status,
            adminNotes,
      ...
      return handleError(error, createErrorContext("bulkReviewReports"));
    }
  }
}
