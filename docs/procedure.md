...
- **Changements récents** : modèle `account` (Better Auth) lié à `user` ; suppression de `session`, `conversation_pin`, `verification`, `workshop_cashback_queue` ; ajout de `magic_link_token`, `deletion_job` (planification des suppressions de compte) ; `audit_log` avec adminId/action/targetId/details.
